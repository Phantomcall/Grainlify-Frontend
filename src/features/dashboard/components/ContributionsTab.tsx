...
const normalizeStatus = (status?: string | null): ContributionStatus => {
  ...
  if (['complete', 'completed', 'merged', 'done', 'paid', 'closed'].includes(normalized)) {
    return 'complete'
  }
  if (
...
const getBadge = (contribution: ProfileContribution) => {
  const value = contribution.badge ?? contribution.issue_number ?? contribution.number ?? null
...
function ContributionError({ theme, onRetry }: { theme: string; onRetry: () => void }) {
  return (
    <div
      role="alert"
...
function ContributionCard({ item, theme }: { item: ContributionCardData; theme: string }) {
  const isComplete = item.status === 'complete'
  const actionLabel =
    item.status === 'applied' || item.status === 'assigned' ? 'See application' : 'See detail'
...
function ContributionColumn({
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        {items.length > 0 ? (
          items.map((item) => <ContributionCard key={item.id} item={item} theme={theme} />)
        ) : (
...
export function ContributionsTab() {
  ...
  const [selectedRewards, setSelectedRewards] = useState<RewardFilter[]>(rewardOptions)
                      ...
                      {projectOptions.length > 0 ? (
                        projectOptions.map((project) => (
                          <button
                            ...
                            onClick={() =>
                              setSelectedProject(selectedProject === project ? '' : project)
                            }
                          ...
                          >
                            <span>{project}</span>
                            {selectedProject === project && <Check className="w-4 h-4" />}
                          </button>
                        ))
                      ) : (
                        <p
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      {rewardOptions.map((reward) => (
                        ...
                        >
                          <span>{reward}</span>
                          {selectedRewards.includes(reward) && <Check className="w-4 h-4" />}
                        </button>
                      ))}
                    </div>
                  </div>
