...
export function ProfilePage({
  ...
  const isSameView = (uid: string | null | undefined, login: string | null | undefined) =>
    ...
    (uid == null &&
      login == null &&
      viewingRef.current.viewingUserId == null &&
      viewingRef.current.viewingUserLogin == null)
  ...
  useEffect(() => {
    ...
    const fetchLed = async () => {
      ...
      try {
        ...
        if (isSameView(requestedUserId, requestedLogin))
          logger.error('Failed to fetch projects led:', error)
      } finally {
  ...
  return (
    <div className="space-y-6">
              ...
              {!isLoadingProfile && (
                <div className="flex items-center gap-3 flex-wrap mb-4">
                  ...
                  <a
                    ...
                    title="GitHub"
                  >
                    <GithubIcon className="w-4 h-4 text-[#c9983a]" />
                  </a>
                  ...
                  <div className="relative flex items-center gap-3 group/stat">
                    ...
                    {isLoadingProfile ? (
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          (profileData?.projects_contributed_to_count ?? 0) > 0 &&
                          setContributorModalOpen(true)
                        }
                        disabled={(profileData?.projects_contributed_to_count ?? 0) === 0}
                        className={`text-[15px] font-medium transition-colors text-left hover:opacity-90 disabled:opacity-60 disabled:cursor-default ${
                          theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                      ...
                      >
                        ...
                        <div className="max-h-[260px] overflow-y-auto p-2 space-y-1">
                          {projects.map((project) => {
                            const name =
                              project.github_full_name.split('/')[1] || project.github_full_name
                            return (
                              <button
                                key={project.id}
                                type="button"
                  ...
                  <div className="relative flex items-center gap-3 group/stat">
                    ...
                    {isLoadingProfile ? (
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          (profileData?.projects_led_count ?? 0) > 0 && setLeadModalOpen(true)
                        }
            ...
            <div className="relative backdrop-blur-[40px] rounded-[28px] border-[3.5px] border-white/50 shadow-[0_15px_60px_rgba(201,152,58,0.5),inset_0_2px_4px_rgba(255,255,255,0.5),0_0_60px_rgba(255,215,0,0.2)] p-10 min-w-[200px] text-center group-hover/rank:scale-105 group-hover/rank:shadow-[0_20px_80px_rgba(201,152,58,0.6),inset_0_2px_4px_rgba(255,255,255,0.6)] transition-all duration-500 bg-gradient-to-br from-[#c9983a]/40 via-[#d4af37]/30 to-[#c9983a]/25">
              ...
              <div className="relative mb-3">
                {isLoadingProfile ? (
                  ...
                  <div className="text-[48px] font-black text-gray-400 leading-none">Unranked</div>
                )}
              </div>
      ...
      <div className="backdrop-blur-[40px] bg-white/[0.12] rounded-[24px] border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-8 relative overflow-hidden group/projects">
        ...
        <div
          className={`relative grid gap-5 ${showAllProjects ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-3'}`}
        >
          {isLoadingProjects ? (
        ...
        {isLoadingActivity ? (
          ...
          >
            ...
            <p className="text-[14px] mt-2">
              Start contributing to verified projects to see your activity here!
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {Object.entries(contributionsByMonth).map(([month, items]) => (
              ...
              >
                ...
                {/* Horizontal Divider */}
                {expandedMonths[month] && <div className="border-t border-white/15" />}
