...
describe('IssuesTab', () => {
  ...
  it('does not show hardcoded category names that are absent from issue data', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: 'Blockchain & Cryptocurrencies' })
  ...
  it('filter options update when issues from a different project are loaded', async () => {
    ...
    expect(
      within(catSection()).queryByRole('button', { name: 'enhancement' })
    ).not.toBeInTheDocument()
    ...
    const NEW_PROJECTS = [
      { id: 'proj-2', github_full_name: 'other-org/other-repo', status: 'verified' },
    ]
    const newIssues = [
      {
        ...ISSUES[0],
    ...
    rerender(<IssuesTab onNavigate={vi.fn()} selectedProjects={NEW_PROJECTS} />)
