...
export function RoleSwitcher({
  ...
  const roles = availableRoles ? allRoles.filter((r) => availableRoles.includes(r.id)) : allRoles
  ...
  return (
    <div
      ...
      className={`w-[80%] lg:w-auto max-w-[800px] lg:max-w-none flex-col lg:flex-row gap-[8px] lg:gap-[2px] lg:h-[44px] items-start p-[2px] rounded-[10px] lg:rounded-[999px] shadow-[0px_6px_6.5px_-1px_rgba(0,0,0,0.36),0px_0px_4.2px_0px_rgba(0,0,0,0.69)] ${
        theme === 'dark' ? 'bg-[#1a1612]' : 'bg-[#8b7d6b]'
      }
        ...
        return (
          <button
            ...
            className={`h-[40px] relative shrink-0 w-full lg:w-[130px] px-3 ${
              isFirst
                ? 'rounded-bl-[10px] lg:rounded-bl-[20px] rounded-br-[4px] rounded-tl-[10px] lg:rounded-tl-[20px] rounded-tr-[4px] '
                : isLast
                  ? 'rounded-bl-[4px] rounded-br-[10px] lg:rounded-br-[20px] rounded-tl-[4px] rounded-tr-[10px] lg:rounded-tr-[20px]'
                  : 'rounded-[4px]'
            } ${
              isActive
                ? theme === 'dark'
          ...
          >
            {/* Button Content */}
            <div
              className={`absolute flex items-center justify-center gap-2 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap ${
                isActive
                  ? 'text-white'
                  : theme === 'dark'
            ...
            >
              <Icon
                ...
                aria-hidden="false"
                className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isActive ? '' : 'opacity-80'}`}
              />

              <span
                className={`text-[11px] font-medium leading-[0] tracking-wide ${
                  isActive
                    ? 'text-shadow-[0px_1px_2px_rgba(0,0,0,0.3)]'
                    : 'text-shadow-[0px_1px_0px_rgba(0,0,0,0.19)]'
                }`}
              >
                {role.label}
              </span>
